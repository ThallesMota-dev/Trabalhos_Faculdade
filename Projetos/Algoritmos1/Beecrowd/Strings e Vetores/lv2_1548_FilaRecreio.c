...
maioria dos alunos. Não só porque as vezes as aulas são cansativas, mas sim porque 
a merenda servida é muito boa, preparada por um chefe italiano muito caprichoso.
...
Em seguida haverá M inteiros distintos Pi (1 ≤ Pi ≤ 1000), onde o i-ésimo inteiro 
indica a nota do i-ésimo aluno.
...
#include <stdio.h>
int main () {
	int n, i, j, k, alunos, nota[1000], copia[1000], nao_mudou, aux;
	scanf("%d", &n);
	for (i=0; i<n; i++) {
		scanf("%d", &alunos);
		for (j=0; j<alunos; j++) {
